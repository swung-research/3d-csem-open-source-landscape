...
Large-scale modelling of three-dimensional (3D) controlled-source electromagnetic (CSEM) responses used to be feasible only for large companies and research consortia. However, this has changed over the last few years, and today there exists a selection of different open-source codes which are up to the task. We have a closer look at four different ones, where in all cases at least the user-facing functionality is written in Python.
% (2) methodology
For the numerical comparison we choose three models, a layered background, ones on its own and ones containing three resistive blocks, and Marlim R3D, a complex, anisotropic, real-world and open-source resistivity model. The analysis includes three different mesh-types, simple tensor meshes, octree meshes, and unstructured tetrahedral meshes. Two of the codes use finite volumes and the other two use finite elements.
% (3) results
...
\itodo{HERE HERE HERE HERE HERE HERE HERE HERE HERE HERE HERE HERE HERE HERE HERE HERE}
\rmk{$\Downarrow$ Re-arrange and re-write all this $\Downarrow$}
...
%
\plot*{results-layered}{width=\textwidth}{
...
\imycom[RR]{I suggest to show y-component for broadside and the errors of the
  ...
  Figure~\ref{fig:results-layered--3}. The runtime and memory requirements are listed
  in Table~\ref{tbl:comp-layered}.

\rmk{$\Uparrow$ Re-arrange and re-write all this $\Uparrow$}

%
\tabl[btp]{comp-layered}{Comparison of runtime, memory, and number of cells used to discretize the layered model.}{
  \centering
\begin{tabular}{lS[table-format=6.0]rS[table-format=4.1]S[table-format=8.0]}
  \toprule
  %
       & {Runtime (s)} & \#Procs & {Memory (GiB)}   & {\#dof} \\
  \midrule
  %
  \custem &   118 & 24 &  97.8 & 1530808 \\ % dof: 3061616 / 2 (Real+Imag -> Complex)
  \emg3d  &   116 &  1 &   0.4 & 4813216 \\
  \petgem &    90 & 24 &  34.2 & 2455868 \\
  \simpeg & 10089 &  4 & 280.3 & 4813216 \\ % nprocs: 1*4 threads
  %
  \bottomrule
\end{tabular}}%
%
...
The block model is a derivation of the \emph{Dublin Test Model 1} from the first EM modelling workshop described by \cite{GJI.13.Miensopust}. We took the same layout of the blocks but adjusted the dimensions and resistivities to a typical marine CSEM problem, as shown in Figure~\ref{fig:model-block}. We additionally added a layered background with vertical transverse isotropy (VTI).
%
\plot*{model-block}{width=.6\textwidth}{
...
The results for the three receiver lines $y=-3,0,3\,$km are shown in Figures~\ref{fig:results-block--3}, \ref{fig:results-block-0}, and \ref{fig:results-block-3}, respectively, and the corresponding required runtime and memory are listed in Table~\ref{tbl:comp-block}. We can conclude from the NRMSD that similar codes have a smaller NRMSD than cross-comparison, hence \emg3d and \simpeg are very similar, as well as are \custem and \petgem. \imycom[DW]{Write in detail when all results are available}
%
\plot*{results-block}{width=\textwidth}{
...
%
\tabl[btp]{comp-block}{Comparison of used runtime, memory, and number of cells
...
\toprule
  ...
  \midrule
  %
  \custem &   158 & 24 & 115.9 & 1654242 \\ % dof: 3308484 / 2  (Real+Imag -> Complex)
  \emg3d  &   115 &  1 &   0.4 & 4813216 \\
  \petgem &    72 & 24 &  35.8 & 2455868 \\
  \simpeg &  9993 &  4 & 280.6 & 4813216 \\ % nprocs: 1*4 threads
  %
  \bottomrule
\end{tabular}}%
%
...
\plot*{model-marlim}{width=\textwidth}{
  ...
  A nice figure would  be Figure 2 in the original paper; however, note that
  the drawn survey line in that figure are plainly wrong.
  \textbf{Air, water, and salt are isotropic, rest has $\lambda=\sqrt{2}$.}
}
%
...
%
\plot*{results-marlim-responses}{width=\textwidth}{
}
%

%
\plot*{results-marlim_2published}{width=\textwidth}{
...
%
\plot*{results-marlim_2ours}{width=\textwidth}{
}
%

%
\plot*{results-marlim_entire}{width=\textwidth}{
}
%
%
\plot*{results-marlim_survey}{width=\textwidth}{
}
%
...
\begin{itemize}
  \item 0.125\,Hz: on the left side, \custem and \emg3d are almost identical,
    ...
    computation, or there is a difference in the model that we did not take
    into account.
  ...
  \item 0.125\,Hz and 0.5\,Hz: on the right side, \custem seems to very similar
    to MR3D.
  \item FD needs comparably a lot of cells
\end{itemize}



%
\tabl[btp]{comp-marlim}{Comparison of used runtime, memory, and number of cells
...
\begin{tabular}{lS[table-format=6.0]rS[table-format=4.1]S[table-format=8.0]}
  ...
  \midrule
  %
  \custem & 4984 & 64 & 558.5 & 4539474 \\ % dof: 9078948 / 2 (Real+Imag -> Complex)
  \emg3d  & 1200 &  1 &   0.5 & 5998992 \\
  \petgem & 3527 & 96 & 437.8 & 4539474 \\
  \simpeg &  422 &  4 &  12.8 &  720146 \\ % nprocs: 1*4 threads
  %
  \bottomrule
\end{tabular}}%
%
