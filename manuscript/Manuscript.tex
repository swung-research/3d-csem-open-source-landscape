...
\begin{enumerate}
  ...
  \item The NRMSD between the FV codes \emg3d and \simpeg is very low except at the boundary. The generally very low NRMSD is because these codes use the same rectilinear mesh for modelling. The increasing NRMSD towards the boundary is because \emg3d uses the PEC boundary condition, while \simpeg uses the PMC boundary condition.
...
\imycom[RR]{Is it possible to force a placement of the tables in the corresponding section? (every table is shifted to the next section, quite unituitive to correlate to the models)}
...
\imycom[RR]{I'd state point 1 at last (most obvious and least important in my opinion)}
\begin{enumerate}
...
\imycom[RR]{The following paragraph is rather discussion content. Before, we mainly restricted to show data only with observations, I would keep this style also here}
...
\imycom[RR]{Mostlyx redundant content to the information introduced before, I think this paragraph can be removed compeltely}
...
\end{tabular}}%
%
\imycom[RR]{tested some new mumps parameters, still no consistent benefit, but the best run was about 15\% faster now and slightly less memory required as I removed some unnecessary interpolation-varaiables overhead}
...
\imycom[RR]{9M real valued vs. 4.5M complex-valued, what is correct here? Anyway, we should also state that even emg3d uses 6M only for the same setup, so why did they (over?)discretized this model so excessively?}
\imycom[DD]{Add a note that the custEM/PETGEM mesh can handle the entire survey, not just this inline/broadline.}
...
\imycom[RR]{changed last sentence to mining + environmental for custEM (actually we will focus way more on gorund/salt water investigations now at LIAG)}
