...
% % == Notes from Wim; to incorporate ==
%
% All implemented
%
% % == Notes form Paulo ==
%
% Very good to know that the Ex responses are well represented. For most of the inversions done in the industry, we use these fields only.
%
% Ez: the mismatch occurs at the longer offsets and below the noise threshold 10-15. EMGS argues now that with the new source, the threshold is pushed to 10-16. But in practice, if the curve follows a smooth decay, we use this data for inversion, and we get meaningful geological results compared to seismic images.
%
% Ey: the broadside component is quite close,  with small differences at long offsets. For the inline component, I've noted that your responses are up to two orders of magnitude higher than MR3D. The four programs you tested give the same amplitudes? It may be a problem with the EMGS software, as EY is always neglected we may have ignored that issue. Simultaneously, it looks that your inline responses are at the same level as the broadside ones.
%
% Bathymetry is also a big issue. It must be well represented to avoid problems.
%
% % == Notes from Evert ==
%
% [T]he fact that the solutions in the four codes are roughly correct with an expected error between 1\% and 10\% makes it totally understandable that the field that is one to two orders of magnitude smaller has no accuracy. You could test this by driving the overall error down by a factor 100. Not sure that is feasible with all codes but if you can do it with one it can be enough when all codes agree sufficiently on the strong field components. Another option can be to increase the cross line component in the error computation.
