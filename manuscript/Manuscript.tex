...
\begin{itemize}
    \item Notes on orthography: As we submit to GJI it should be British English. But I settled on discretize with a «z». My dictionary says that discreti{\color{red}\bfseries z}e is AE and discreti{\color{red}\bfseries s}e is BE, but that «z» is also used in BE in mathematical contexts. Is that fine or any other opinions?
    \imycom[LJH]{discretize is good with me}
    \item Regarding submission, {\color{red}\bfseries please let me know}:
      \begin{itemize}
        \item If you think the \textbf{order of authors} should be different. The last thing we want in this collaboration is that someone feels unhappy about that: \itodo{No comment from RR; OCR said OK, LJH: +1 looks good to me :)}
        \item If you have \textbf{another manuscript under revision with GJI}. When submitting to GJI one has to indicate when you have other manuscripts currently under review with GJI: \itodo{No comments from Raphael; Octavio, No from Lindsey}
        ...
        \imycom[LJH]{+1, fine by me to transfer the whole repo over with git history (either way is fine by me) }
        }
        \item \textbf{Wanted and not wanted reviewers:} Let me know if you have any names for these. I do not have any \emph{not wanted}, but I do have a few I will put on the \emph{wanted} list, mainly people I would be interested to have their feedback and that have knowledge about 3D modelling:
          \begin{itemize}
            \item Kerry Key (domain knowledge with \texttt{MARE3DEM}, also experience with open-sourcing with \texttt{MARE2DEM}); \itodo{RR and QCR and LJH agreed}
...
The actual differences in meshing is shown in the next two figures, in Figure~\ref{fig:results-marlim_big} for almost the entire computational domain, and in Figure~\ref{fig:results-marlim_survey} a zoom-in to the survey domain, the domain of interest. The dimension of the computational domain varies for the different meshes:
% emg3d:         x = 327 to 454 km; z =  -57 to 83 km
% SimPEG:        x = 288 to 492 km; z = -123 to 41 km
% custEM/PETGEM: as shown in the image (+/- 25 km)
\plot*{results-marlim_big}{width=\textwidth}{Meshes (left column), broadside $E_x$ field (middle column), and broadside $E_y$ field (right column) for a rectilinear mesh (top row), octree mesh (middle row), and tetrahedra mesh (bottom row). The shown dimensions shows the entire computational domain for the tetrahedra mesh, whereas the entire rectilinear and octree meshes are bigger.}
%
The top rows show the rectilinear mesh of \emg3d, the middle rows the octree mesh of \simpeg, and the bottom rows the tetrahedra mesh of \custem and \petgem. The actual mesh, the $|E_x|$ field, and the $|E_y|$ field are shown in the left, middle, and right columns, respectively.

%
\plot*{results-marlim_survey}{width=\textwidth}{Meshes (left column), $E_x$ field (middle column), and $E_y$ field (right column) for a rectilinear mesh (top row), octree mesh (middle row), and tetrahedra mesh (bottom row) for the survey domain (3$\times$ vertical exaggeration).}
...
\end{itemize}
\plot*{results-marlim_allmodels}{width=\textwidth}{TODO (3$\times$ vertical exaggeration).}
