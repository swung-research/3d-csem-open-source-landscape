...
A few clarifying words on abbreviations: For the boundary conditions (BC) in the comparison table we use the abbreviations PEC and PMC, which stand for perfect electric conductor and perfect magnetic conductor, respectively. Another abbreviation used in the tables is dof for degree of freedom, which is equivalent to the size of the SLE we are solving. Finally, we abbreviate runtime with CPU and memory with RAM. CPU refers throughout to the wall- or real-world time used from start to end of a computation, whether or not there are several processes running in parallel or not (hence it is not actual computation time, which would be significantly higher for parallel computations). RAM, on the other hand, refers to the maximum used memory at any point of the computation.
