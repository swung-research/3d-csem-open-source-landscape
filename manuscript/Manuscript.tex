...
(FV) \citep{EM.90.Madsen, ECP.07.Haber, GEO.14.Jahandari, PIER.01.Clemens,
GP.06.Mulder}. And these are just the most common ones.
...
\tabl[btp]{codecomparison}{All codes solve Maxwell's equations in its
  ...
  \imycom[DW]{PEC/PMC instead of Dirichlet/Neumann?}
}{
  \centering
  \footnotesize
\begin{tabularx}{\linewidth}{lXXXX}
  ...
  BC & Neumann; Dirichlet & PEC & PEC & Dirichlet; Neumann \\
...
%
\tabl[btp]{machines}{List of computer and operating system on which the
  different codes were run, together with the operator.}{
  \centering
  \footnotesize
...
is therefore to compare \emg3d/\simpeg (the two FD ones), \custem/\petgem (the
...
this is due to a practical difference in the meshing: \textbf{Rewrite, Comments
Raphael} “\emph{Yes, cutting the layers has an influce at offsets greater
  distance X (see example \#2 thesis) - depending on the boundary mesh. I'm
  sure that's the reason. I could also add a version with a way larger
  horizontal layered earth extent, but this would require way more resources -
  unstructured meshes are just not competitive to regular grids for regular
geopmetries in such cases as discussed before}“
