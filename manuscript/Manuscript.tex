...
%
\\~\\ \# \# Alternative \# \# \\
Large-scale modelling of three-dimensional controlled-source electromagnetic (CSEM) responses used to be feasible only for large companies and research consortia. This has changed over the last few years, and today there exists a selection of different open-source codes. Using four different codes in the Python ecosystem we model responses for three increasingly complex models. The verification of the results with semi-analytical solutions for a simple case as well as the validation between the codes and with results from the industry for more complex models show that the open-source codes are able to compute CSEM responses for challenging, large-scale models. Our comparison includes finite-element and finite-volume codes, using structured tensor and octree meshes as well as unstructured tetrahedra meshes. Our results show further that accurate responses can be obtained independently of the chosen method and the chosen mesh. The difficult part, however, is to appropriately discretize the model. The runtime and memory requirements, however, vary greatly between different codes. But we have found that much more time was spent on creating the appropriate input files than running the actual computation. We provide the three models and the corresponding responses of four codes, which can be used for verification of new and existing codes. The collaboration of four code maintainer trying to achieve the same task brought in the end all four codes a significant step further. We hope that these results may be useful for the entire CSEM community at large, and we invite and encourage the community to make more code, modelling scripts, and results publicly available.
\end{abstract}
...
%
The NRMSD is generally below a few percent, except for large offsets. An interesting feature is that the NRMSD between \emg3d and \simpeg is very low except at the boundary. The reason is that the codes use the same tensor mesh for modelling, but while \emg3d uses the PEC boundary condition, while \simpeg uses the PMC boundary condition.
\mycom[DW]{Lindsey, what about the different formulations we use?}
\todo{expand the analysis of the Block Model -- any ideas / inputs?}
...
\itodo{Discussion poins from Evert: How can you attribute differences to the solver? There are mesh element effects as well and the way Maxwell's equations are discretized plays a role too.}
