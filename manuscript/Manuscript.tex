...
\tabl[btp]{codecomparison}{All codes solve Maxwell's Equations in its
  differential form (DE) and with the quasistatic approximation. We use two
  finite element codes and two finite difference codes; the finite element
  codes both make use of the FEniCS solver \citep{CSE.15.Alnaes}, whereas
  \simpeg can use either solvers implemented in SciPy or the PARDISO solver
  \citep{FGCS.04.Schenk}; \emg3d, on the other hand, is a solver on its own.}{
  \centering
  \small
\begin{tabular}{lcccc}
  \toprule
