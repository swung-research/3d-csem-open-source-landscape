...
\mycom[RR]{GMD looks very interesting, my only concern is that it might not be
in the scope of EM modelers, but we can probably reach modeleres from other
communities as well.}
\mycom[OCR]{I also find GMD very interesting, in fact it was my first option to publish the first petgem version (2 years ago). The answer was: "manuscript outside of journal's scope". In this sense I agree with Raphael. My vote is for SIG}
...
\imycom[OCR]{In general, I believe that the content is adequate and the ideas are clear and in accordance with the objective that we have initially set. However, I want to highlight the task of being careful when justifying the absence of the SimPEG results for the marlim model (I have read it in slack, although it may not be definitive). I believe that this may or may not be a weak point depending on how we explain it. I know that perhaps it is not serious, I only make this comment so that the speech justifies the facts. }
...
\mycom[OCR]{I have rearranged some references according to the year of publication (here and in the rest of the paper)}
...
GJI.11.Schwarzbach,GEO.12.daSilva, GJI.13.Grayver, GJI.13.Puzyrev, SEG.16.Zhang},
and finite volumes (FV) \citep{EM.90.Madsen, PIER.01.Clemens, GP.06.Mulder, ECP.07.Haber, GEO.14.Jahandari}. There are also many different types of
...
between direct solvers \citep{GEO.09.Streich, GP.14.Chung,
GEO.14.Jaysaval, GEO.15.Grayver, SEG.15.Oh, GJI.18.Wang}, indirect solvers \citep{GP.06.Mulder, GJI.15.Jaysaval}, or a combination of both, so-called hybrid solvers
...
\imycom[DW]{Others coming to your mind?\imycom[RR]{no thoughts about geophysics,
  but there are many projects for related topics such as gmt, gis, mesh-gen}
...
\citep{CAG.18.CastilloReyes, GJI.19.CastilloReyes}, and \simpeg \citep{CAG.15.Cockett}. All four
...
meaning that they come with both, an open-source license and an online-hosted
version-control system with tracking possibilities (raising issues, filing pull
requests). All developments comprise an extensive online documentation with
many examples and have continuous integration to some degree. Newer
package-management systems such as Conda, Docker, or pip significantly
supported us to make our codes easily available and installable without any
user-side compilations. Each one can be downloaded and installed with a single
command.
...
quasistatic or\mycom[DW]{are we all using the diff. approx?}\mycom[OCR]{petgem consider the weak work of maxwell equations in their diffusive form (quasi-static approximation)} diffusive
...
\petgem is a parallel code for frequency-domain 3D CSEM data from marine and land surveys. The high-order edge FE method (HEFEM) is used for the discretization of the governing equations in its diffusive form. This technique provides a suitable mechanism to obtain stable numerical solutions as well as a good trade-off between number of dof\mycom[OCR]{I assume we have previously mentioned the term dofs} and computational effort. The current implementation supports up to six-order tetrahedral vector basis functions. Moreover, because the HEFEM belongs to the FE family, the unstructured meshes can be used efficiently on complex geometries (e.g., models with topography and bathymetry). 
...
modeling routines and support for multi-source set-ups will be implemented. For the long-term, the obvious next step is to move to 3D CSEM data inversion.
...
good-quality discretisation. Therefore, the FE codes \custem and \petgem used the
same tetrahedral mesh with a horizontal layered-earth extent of 30\,km from the
origin, surrounded by a ten times larger halfspace-like boundary mesh with
...
realistic model and the data were computed by an industry-proofed
code. Additionally, that code is a time-domain code, whereas the four codes
under consideration here compute the results all in the frequency-domain.
\imycom[RR]{I would delete this sentence, not that important and a bit
redundant.\imycom[DW]{Did you refer to the last sentence or the one before? I
think the last sentence contains quite important info. .\imycom[OCR]{If possible, I would like to uphold this sentence. I agree with dieter, it contains relevant information.}}}

The detailed reservoir model consists of 1022 by 371 by 1229 cells\mycom[OCR]{I prefer this notation $1022 \times 371 \times 1229$. Nothing special, just format}, totalling
to almost 466 million cells, where each cell has dimensions of 25 by 75 by
5\,m. For the computation the model is upscaled to 515 by 563 by 310 cells,
totalling to almost 90 million cells, where each cell has dimensions of 100 by
100 by 20\,m.
...
\tabl[btp]{comp-marlim}{Comparison of used CPU and RAM and of number of cells
  ...
  out once we all submitted the final versions of the results.\imycom[OCR]{I can help you to fill this table and with its analysis}}}{
  \centering
\begin{tabular}{c c c c c}  %{ls[table-format=4.2]rs[table-format=4.2]rl}
  \toprule
  %
...
\imycom[DW]{I would like to capture as many as possible of the (I) differences,
  ...
  \imycom[OCR]{ I think an important point is the user experience (modeler). I know this is an intangible element but perhaps we could add a line about it. It is my opinion}}
...
Fona-r4 under grant 033R130D. The work of O.C-R. has received funding from the \textit{European Union\textquotesingle s Horizon 2020 programme} under the \textit{Marie Sklodowska-Curie} grant agreement N$^\circ$ 777778. Further, the development of \petgem has received funding from the \textit{European Union\textquotesingle s Horizon 2020 programme}, grant agreement N$^\circ$ 828947, and from the Mexican Department of Energy, CONACYT-SENER Hidrocarburos grant agreement N$^\circ$ B-S-69926. Furthermore, O.C-R. has been 65\% cofinanced by the European Regional Development Fund (ERDF) through the Interreg V-A Spain-France-Andorra program (POCTEFA2014-2020). POCTEFA aims to reinforce the economic and social integration of the French-Spanish-Andorran border. Its support is focused on developing economic, social and environmental cross-border activities through joint strategies favouring sustainable territorial development.\mycom[OCR]{My apologies for these long acks, but they are the projects in which I am participating and the BSC makes a lot of emphasis on complying verbatim with the thansk sentences (too rigorous in my opinion)}
