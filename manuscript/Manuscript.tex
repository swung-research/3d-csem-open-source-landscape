...
\paragraph{Comments Raphael} “\emph{Yes, cutting the layers has an influce at
  offsets greater distance X (see example \#2 thesis) - depending on the
  boundary mesh. I'm sure that's the reason. I could also add a version with a
  way larger horizontal layered earth extent, but this would require way more
  resources - unstructured meshes are just not competitive to regular grids for
  regular geopmetries in such cases as discussed before}“
