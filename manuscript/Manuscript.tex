...
\tabl[btp]{codecomparison}{Caption.}{
  \centering
  \small
\begin{tabular}{lcccc}
  \toprule
  %
  Code & \custem & \emg3d & \petgem & \simpeg  \\
  \midrule
  %
  ...
  Mesh & FE & FD & FE & FD; OcTree; CylMesh \\
  BC & ? & PEC & ? & ? \\
  Methods & ? & Diffusion & ? & ? \\
  Solver & FEniCS$^\dagger$ & \emg3d & FEniCS$^\dagger$ &
           PARDISO$^\ddagger$; SciPy \\
  %
  \bottomrule
\end{tabular}}%
%
As is visible from Table~\ref{tbl:codecomparison}, we use two finite element
codes and two finite difference codes; the finite element codes both make use
of the FEniCS solver \citep{CSE.15.Alnaes}, whereas \simpeg can use either
solvers implemented in SciPy, or the PARDISO solver \citep{FGCS.04.Schenk};
\emg3d, on the other hand, is a solver on its own.

\imycom[DW]{
Each code should be briefly summarized by four points:\\
- Brief intro and main features\\
- Main selling point\\
- Planned features\\
- List (same list for all)\\
}
...
\imycom[DW]{Write here your fundings; and potentially also minor contributors etc.}
...
All files to reproduce the shown results and more are available at
\todo{zenodo}\dots.
